...
const HiraConfigurationTab = ({
  ...
  const getRiskConfigById = async () => {
    try {
      const res = await axios.get(`/api/riskconfig/getconfigbyid/${id}`);
  ...
  const handleSubmitHiraConfig = async () => {
    try {
      ...
      if (res.status === 200 || res.status === 201) {
        enqueueSnackbar(
          `Hira Configuration ${edit ? "Updated" : "Created"} Successfully`,
          {
            variant: "success",
          }
        );
      ...
      console.error("Error in handleSubmitHiraConfig", error);
