...
fields = [
    ...
    create_varchar_field(name="organizationId",
                         max_length=255),  # Organization ID
    create_json_field(name="metaData"),  # JSON field storing all audit details
    # create_json_field(name="chunkData"),  # JSON field storing raw chunked text
    create_varchar_field(name="chunkData", max_length=65535),
    # Updated embedding size to 1536
    create_float_vector_field(name="embeddingData", dim=1536)
]

# Create the collection schema
auditReportSchema = create_collection_schema(
    fields=fields,
    description="Optimized schema for storing Audit Reports with metadata, chunkData, and embeddings",
